...
export async function getReviewOrderDetailAction(orderId: string) {
  try {
    ...
    const { data, error } = await supabase
      .from("review_orders")
      .select("id, user_id, status, facebook_url, business_name, order_type, review_type, reaction_type, review_content, review_instructions, proof_of_completion, credits_consumed, quantity, gender, assigned_employee_id, assigned_at, completed_at, admin_verification_status, admin_verified_at, client_feedback, content, comment_text, comment_count, completed_comments, photo_urls, created_at, updated_at")
      .eq("id", orderId)
      .eq("user_id", auth.user.id)
      .single();
    ...
    const normalizedData = {
      ...
      photoReviews: data.order_type === 'COMMENT_WITH_PHOTO' && reviewUrlsData && reviewUrlsData.length > 0
        ? (() => {
            const firstUrl = reviewUrlsData[0];
