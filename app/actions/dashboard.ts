...
export async function getClientProfilesAction() {
  try {
...
export async function requestProfileChangeAction(profileId: string) {
  try {
    const auth = await requireAuth();
    if (!auth.success) return auth;
    ...
    return { success: false, error: error?.message || "Failed to request profile change" };
  }
}
