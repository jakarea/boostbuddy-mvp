            ...
            {pendingClientsCount === 0 ? (
              ...
              <>
                ...
                <div className="md:hidden flex flex-col gap-2 p-3">
                  {pendingClients.map((client) => (
                    <div key={client.id} className="bg-zinc-50 dark:bg-zinc-950/50 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3 flex flex-col gap-2">
                      <div>
              ...
              <>
                        ...
                        return (
                          <TableRow key={p.id} className="border-b border-zinc-200 dark:border-zinc-800">
                            <TableCell className="text-xs font-bold text-zinc-800 dark:text-zinc-200">
                              <Link href={`/a/profiles?action=edit&id=${p.id}`} className="hover:underline flex items-center gap-1 text-[#168BB0] dark:text-[#45B0D2]">
                                {p.profile_name}
                ...
                <div className="md:hidden flex flex-col gap-2 p-3">
                  {expiringProfiles.map((p) => {
                    const days = calculateDaysRemaining(p.expiration_date);
                    return (
                      <div key={p.id} className="bg-zinc-50 dark:bg-zinc-950/50 border border-zinc-200 dark:border-zinc-800 rounded-lg p-2.5 flex flex-col gap-1.5">
                        <div className="flex justify-between items-start">
                          <Link href={`/a/profiles?action=edit&id=${p.id}`} className="font-bold text-xs hover:underline flex items-center gap-1 text-[#168BB0] dark:text-[#45B0D2]">
                            {p.profile_name}
                            <ArrowRight className="h-3 w-3" />
                          </Link>
                          <Badge className="bg-red-500/15 text-red-700 dark:text-red-400 border-red-500/25 shadow-none text-[9px] font-bold">
