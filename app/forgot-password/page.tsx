...
import { Globe, Lock, ArrowLeft, MailOpen, AlertCircle, CheckCircle2 } from "lucide-react";
  ...
  useEffect(() => {
    if (state) {
      if (state.success) {
        ...
        setErrorMsg(state.error || t("failed_send"));
        setSuccess(false);
      }
    }
  }, [state, t]);
      ...
      <div className="flex flex-col p-6 sm:p-12 justify-center items-center relative bg-zinc-50 dark:bg-zinc-950">
        ...
        <div className="w-full max-w-sm space-y-6">
          ...
          <div className="space-y-2 text-center lg:text-left">
            <Button
              variant="link"
              className="text-zinc-500 hover:text-zinc-900 dark:hover:text-white px-0 h-6 text-xs gap-1.5 font-bold"
              onClick={() => router.push("/")}
            >
              <ArrowLeft className="h-4 w-4" />
              {t("back_to_login")}
            </Button>
            <h2 className="text-xl font-extrabold tracking-tight mt-2">{t("reset_title")}</h2>
              ...
              {success ? (
                ...
                <div className="space-y-4 py-4 text-center">
                  <div className="flex justify-center">
                    <div className="p-3 bg-emerald-500/15 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded-full animate-bounce">
                      <MailOpen className="h-8 w-8" />
                    </div>
                  </div>
                  <div className="space-y-1">
                    <h3 className="font-extrabold text-sm text-zinc-800 dark:text-zinc-200">{t("email_dispatched_title")}</h3>
                    <p className="text-xs text-zinc-500 leading-relaxed">
                      {t("email_dispatched_desc", { email })}
                    </p>
                  </div>
                  <Button 
                    className="w-full bg-zinc-900 hover:bg-zinc-800 text-white font-bold h-9 text-xs cursor-pointer shadow-sm mt-3"
                    onClick={() => router.push("/")}
                  >
                    {t("return_to_login")}
                  </Button>
                ...
                <form action={formAction} className="space-y-3">
                  <div className="space-y-1.5">
                    <Label htmlFor="r-email" className="text-xs font-bold text-zinc-700 dark:text-zinc-300">
                      {t("email_address")}
                    </Label>
                    <Input
                      id="r-email"
