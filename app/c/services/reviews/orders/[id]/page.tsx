...
export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const hasPhotos = order.photoUrls && order.photoUrls.length > 0;
      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-6">
            ...
            {user?.role !== 'CLIENT' && (
              <div className="flex-1 min-w-[180px]">
                <p className="text-xs text-zinc-500 dark:text-zinc-400 uppercase tracking-wide font-semibold mb-1">
                  {t("orders.assigned_employee", "Assigned Employee")}
                </p>
        ...
        {hasUrls ? (
          ...
          <Card className="p-4">
            <div className="flex items-center gap-2 mb-4">
              <LinkIcon className="h-5 w-5 text-blue-600 dark:text-blue-400" />
              <h3 className="font-semibold text-lg">{t("orders.facebook_url", "Facebook URL")}</h3>
            </div>
        ...
        {hasUrls && order.reviewUrls.some((u: any) => u.reviewContent) ? (
          <Card className="p-4">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-lg">{t("orders.reviews", "Reviews")}</h3>
            </div>
            <div className="space-y-2">
          ...
          <Card className="p-4">
            <div className="flex items-center gap-2 mb-4">
              <FileText className="h-5 w-5 text-zinc-600 dark:text-zinc-400" />
              <h3 className="font-semibold text-lg">{t("orders.review_content", "Review Content")}</h3>
            </div>
            <div className="space-y-2">
              <div className="flex items-start gap-3 p-3 bg-zinc-50 dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => copyToClipboard(order.content, "Review")}
                  className="h-8 w-8 p-0 shrink-0 cursor-pointer"
                >
                  <Copy className="h-4 w-4" />
                </Button>
        ...
        {(order.orderType === "COMMENT" || order.orderType === "COMMENT_WITH_PHOTO") && order.commentText && order.commentText.trim() && (
          <Card className="p-4">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-lg">{t("orders.comment_text", "Comment Text")}</h3>
            </div>
        ...
        {hasPhotos && (
          <Card className="p-4">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-lg">{t("orders.photos", "Photos")} ({order.photoUrls.length})</h3>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <Button
                      ...
                      className="absolute top-3 right-3 h-8 w-8 p-0 bg-white/90 dark:bg-zinc-800/90 shadow-sm cursor-pointer"
                    >
                      <Copy className="h-4 w-4" />
                    </Button>
                    <Button
                      ...
                      className="absolute bottom-12 right-3 h-8 w-8 p-0 bg-white/90 dark:bg-zinc-800/90 shadow-sm cursor-pointer"
                    >
