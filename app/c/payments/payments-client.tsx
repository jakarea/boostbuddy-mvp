...
export default function PaymentsClient({ initialOrders, activeServices, billingInfo, invoices, profiles }: PaymentsClientProps) {
  ...
  const goToPage = (page: number) => {
    ...
    router.push(`/c/payments?${params.toString()}`);
    setCurrentPage(page);
  };
  ...
  const filteredOrders = useMemo(() => {
    ...
    return initialOrders.filter((ord) => {
      const serviceName = getServiceName(ord).toLowerCase();
      const status = ord.status.toLowerCase();
      const type = ord.type.toLowerCase();
      const sessionId = ord.stripe_session_id?.toLowerCase() || "";
      return (
          ...
          <div className="flex items-center gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder={t("search_placeholder", "Search orders...")}
