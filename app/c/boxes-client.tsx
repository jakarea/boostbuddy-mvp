...
const TwoFactorTimer: React.FC<{ secret: string }> = ({ secret }) => {
  const { t } = useTranslation("client_boxes");
  ...
  return (
    <div className="flex items-center justify-between p-2.5 rounded-lg bg-zinc-100 dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-800 text-xs font-mono">
      <div className="flex items-center gap-2">
        <span className="text-zinc-400 font-bold uppercase tracking-wider text-[9px] block">{t("card_2fa", { defaultValue: "2FA" })}</span>
...
export default function BoxesClient({ initialBoxes }: { initialBoxes: BoxAccountRecord[] }) {
  const { t } = useTranslation("client_boxes");
  ...
  }, [isRenewModalOpen, selectedBox, selectedServiceId]);

  const handleOpenRenewModal = (box: any) => {
    setSelectedBox(box);
    setIsRenewModalOpen(true);
    const defaultSrvId = box.service_id || (services.length > 0 ? services[0].id : "");
    setSelectedServiceId(defaultSrvId);
  };

  const handleProceedCheckout = () => {
    if (!selectedBox || !selectedServiceId) return;
    router.push(`/checkout?type=renewal&profileId=${selectedBox.id}&serviceId=${selectedServiceId}`);
  };
  ...
  const handleRequestChange = async (boxId: string) => {
    ...
    if (result.isConfirmed) {
      startTransition(async () => {
        await requestProfileChangeAction(boxId);
        success(t("alert_requested_text", { defaultValue: "Change requested successfully" }));
      });
    }
  };

  // The server action already filters for the current user and valid statuses
  const clientBoxes = initialBoxes;

  // Calculate paginated results
  const paginatedBoxes = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    return clientBoxes.slice(startIndex, startIndex + itemsPerPage);
  }, [clientBoxes, currentPage, itemsPerPage]);

  const totalPages = Math.ceil(clientBoxes.length / itemsPerPage);
  ...
  // Check if any boxes are expiring soon
  const expiringSoonBoxes = clientBoxes.filter(p => {
    const days = getDaysRemaining(p.expiration_date);
    return p.status === "ACTIVE" && days !== null && days >= 0 && days <= 7;
  });

  // Trigger warning toast for expiring soon boxes
  useEffect(() => {
    if (expiringSoonBoxes.length > 0) {
      warning(`${t("exp_warning_title", { defaultValue: "Boxes Expiring Soon" })} ${t("exp_warning_desc", { defaultValue: "{{count}} boxes expiring soon", count: expiringSoonBoxes.length })}`, 8000);
    }
  }, [expiringSoonBoxes.length, t, warning]);
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-4 sm:pb-5">
        <div className="min-w-0">
          <h1 className="text-lg sm:text-2xl md:text-3xl font-extrabold tracking-tight">{t("title", { defaultValue: "My Boxes" })}</h1>
          <p className="text-xs sm:text-sm text-zinc-500 mt-1">
            {t("subtitle", { defaultValue: "Manage your browser profile boxes" })}
          </p>
        </div>
        <Button
          className="bg-[#168BB0] hover:bg-[#0F7493] text-white font-bold cursor-pointer h-10 text-sm w-full sm:w-auto shrink-0"
          onClick={() => router.push("/c/boxes/buy")}
        >
          {t("buy_new_box", { defaultValue: "Buy New Box" })}
        </Button>
      </div>
      ...
      <Card className="bg-zinc-100 dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800/80">
        <CardContent className="p-3 sm:p-4 flex gap-3 sm:gap-3.5 items-start text-xs sm:text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">
          ...
          <div className="space-y-1">
            <h4 className="font-bold text-xs sm:text-sm text-zinc-900 dark:text-zinc-200">{t("instructions_title", { defaultValue: "How to Use Your Boxes" })}</h4>
            <p className="leading-relaxed">
              {t("instructions_desc", { defaultValue: "Each box contains a browser profile with credentials. Use the IXBrowser reference codes to access your profiles." })}
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      {clientBoxes.length === 0 ? (
        <div className="text-center py-8 sm:py-12 border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-lg p-4 sm:p-6 bg-white dark:bg-zinc-900/40">
          ...
          <h3 className="font-bold text-sm sm:text-base text-zinc-800 dark:text-zinc-200">{t("no_boxes_title", { defaultValue: "No Boxes Yet" })}</h3>
          <p className="text-xs sm:text-sm text-zinc-500 max-w-sm mx-auto mt-1 sm:mt-2 leading-relaxed">
            {t("no_boxes_desc", { defaultValue: "You haven't purchased any boxes yet. Get started by buying your first box." })}
          </p>
          <Button
            size="sm"
            ...
            onClick={() => router.push("/c/boxes/buy")}
          >
            {t("go_to_store", { defaultValue: "Go to Store" })}
          </Button>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            {paginatedBoxes.map((p) => {
              ...
              <Card key={p.id} className={`bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col justify-between overflow-hidden relative ${p.status === "REQUEST_CHANGE" ? "ring-2 ring-orange-500/30 bg-orange-50/50 dark:bg-orange-950/10" : isExpiringSoon || isExpired ? "ring-1 ring-[#168BB0]/20" : ""}`}>
                ...
                {p.status === "REQUEST_CHANGE" && (
                  <div className="bg-orange-500/10 text-orange-700 dark:text-orange-300 border-b border-orange-500/15 px-3 sm:px-4 py-1 sm:py-1.5 text-[10px] sm:text-[11px] font-semibold">
                    ...
                    {t("banner_change_requested", { defaultValue: "Change Requested" })}
                  </div>
                )}
                ...
                {isExpiringSoon && (
                  <div className="bg-[#168BB0]/10 text-[#0F7493] dark:text-[#45B0D2] border-b border-[#168BB0]/15 px-3 sm:px-4 py-1 sm:py-1.5 text-[10px] sm:text-[11px] font-semibold flex items-center gap-1.5">
                    ...
                    {daysLeft === 1 ? t("banner_expiring_one", { defaultValue: "Expiring in 1 day", count: daysLeft }) : t("banner_expiring", { defaultValue: "Expiring in {{count}} days", count: daysLeft })}
                  </div>
                )}
                {isExpired && (
                  <div className="bg-red-500/10 text-red-700 dark:text-red-300 border-b border-red-500/15 px-3 sm:px-4 py-1 sm:py-1.5 text-[10px] sm:text-[11px] font-semibold flex items-center gap-1.5">
                    <AlertTriangle className="h-3 w-3 sm:h-3.5 sm:w-3.5" />
                    {t("banner_expired", { defaultValue: "Box Expired" })}
                  </div>
                )}
                  ...
                  <div className="flex justify-between items-start gap-2">
                    <div className="space-y-0.5 sm:space-y-1 min-w-0">
                      ...
                      <CardDescription className="text-[10px] sm:text-[11px] text-zinc-500">
                        {t("assigned_on", { defaultValue: "Assigned on {{date}}", date: formatDate(p.assignment_date) || "N/A" })}
                      </CardDescription>
                    </div>
                    <Badge variant="outline" className={`${statusBadgeColor} font-bold text-[8px] sm:text-[9px] uppercase tracking-wider shrink-0`}>
                      {isExpired ? t("card_expired", { defaultValue: "Expired" }) : tStatus(p.status, { defaultValue: p.status })}
                    </Badge>
                  </div>
                </CardHeader>
                ...
                <CardContent className="space-y-3 sm:space-y-4 pb-3 sm:pb-4 px-3 sm:px-4">
                  ...
                  <div className="space-y-2 sm:space-y-2.5">
                    <span className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider block">
                      {t("card_credentials", { defaultValue: "Box Credentials" })}
                    </span>
                  ...
                  {p.two_factor_secret && (
                    <div className="space-y-1 sm:space-y-1.5">
                      <span className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider block">
                        {t("card_2fa", { defaultValue: "2FA Code" })}
                      </span>
                  ...
                  <div className="space-y-2 bg-zinc-50 dark:bg-zinc-950 p-2 sm:p-3 rounded-lg border border-zinc-200 dark:border-zinc-800">
                    <span className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider block">
                      {t("card_ixbrowser", { defaultValue: "IXBrowser Reference" })}
                    </span>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 sm:gap-4 text-xs">
                      <div>
                        <div className="text-[10px] text-zinc-500">{t("card_profile_id", { defaultValue: "Profile ID" })}</div>
                        <div className="font-bold font-mono text-zinc-800 dark:text-zinc-200 flex items-center justify-between mt-0.5">
                          <span className="truncate">{p.ixbrowser_profile_id || t("card_not_set", { defaultValue: "Not set" })}</span>
                          {p.ixbrowser_profile_id && (
                            <Button size="icon" variant="ghost" title={t("btn_copy", { defaultValue: "Copy" })} className="h-5 w-5 text-zinc-500" onClick={() => copyText(p.ixbrowser_profile_id || "", t("card_profile_id", { defaultValue: "Profile ID" }))}>
                      ...
                      <div>
                        <div className="text-[10px] text-zinc-500">{t("card_group", { defaultValue: "Group" })}</div>
                        <div className="font-bold font-mono text-zinc-800 dark:text-zinc-200 flex items-center justify-between mt-0.5">
                          <span className="truncate">{p.ixbrowser_group || t("card_not_set", { defaultValue: "Not set" })}</span>
                          {p.ixbrowser_group && (
                            <Button size="icon" variant="ghost" title={t("btn_copy", { defaultValue: "Copy" })} className="h-5 w-5 text-zinc-500" onClick={() => copyText(p.ixbrowser_group || "", t("card_group", { defaultValue: "Group" }))}>
                              <Copy className="h-2.5 w-2.5" />
                            </Button>
                          )}
                        </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-2 text-xs sm:text-sm border-t border-zinc-100 dark:border-zinc-800/80 pt-2 sm:pt-3">
                    <span className="text-zinc-500">{t("card_expires", { defaultValue: "Expires" })}</span>
                    <span className={`font-bold ${isExpiringSoon || isExpired ? "text-[#168BB0] dark:text-[#45B0D2]" : "text-zinc-700 dark:text-zinc-300"}`}>
                      ...
                      {p.expiration_date && (
                        <span className="text-[9px] sm:text-[10px] font-medium ml-1.5 text-zinc-400">
                          ({isExpired ? t("card_expired", { defaultValue: "Expired" }) : (daysLeft === 1 ? t("card_days_left_one", { defaultValue: "1 day left", count: daysLeft }) : t("card_days_left", { defaultValue: "{{count}} days left", count: daysLeft }))})
                        </span>
                      )}
                    </span>
                  </div>
                ...
                <CardFooter className="bg-zinc-50 dark:bg-zinc-900/50 border-t border-zinc-100 dark:border-zinc-800 p-2 sm:p-3 flex flex-col gap-2">
                  <div className="flex flex-col sm:flex-row gap-2 w-full">
                    <Button
                      ...
                      onClick={() => info(t("alert_launch_text", { defaultValue: "Launch IXBrowser and enter the profile ID to access this box." }))}
                    >
                      <ExternalLink className="h-3.5 w-3.5 sm:h-4 sm:w-4 mr-1.5 text-zinc-400 shrink-0" />
                      <span className="hidden sm:inline">{t("btn_launch_how", { defaultValue: "How to Launch" })}</span>
                      <span className="sm:hidden">{t("btn_launch", { defaultValue: "Launch" })}</span>
                    </Button>
                    <Button
                      variant="outline"
                    ...
                    >
                      ...
                      {t("btn_support", { defaultValue: "Support" })}
                    </Button>
                    <Button
                    ...
                    >
                      ...
                      <span className="hidden sm:inline">{t("btn_renew_box", { defaultValue: "Renew Box" })}</span>
                      <span className="sm:hidden">{t("btn_renew", { defaultValue: "Renew" })}</span>
                    </Button>
                  </div>
                  <Button
                    variant="outline"
                  ...
                  >
                    ...
                    {p.status === "REQUEST_CHANGE" ? t("btn_change_requested", { defaultValue: "Change Requested" }) : t("btn_request_change", { defaultValue: "Request Change" })}
                  </Button>
                </CardFooter>
              </Card>
            );
            ...
            <DialogContent className="sm:max-w-[450px] bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 text-zinc-900 dark:text-zinc-100 p-6 rounded-xl">
              <DialogHeader>
                <DialogTitle className="text-lg font-extrabold tracking-tight">
                  {t("modal_renew_title", { defaultValue: "Renew / Upgrade Box" })}
                </DialogTitle>
                <DialogDescription className="text-xs text-zinc-500 mt-1">
                  {t("modal_renew_desc", { defaultValue: "Select a package to extend or upgrade your box subscription." })}
                </DialogDescription>
              </DialogHeader>
                ...
                <div className="space-y-4 py-4 text-xs">
                  <div className="bg-zinc-50 dark:bg-zinc-950 p-3 rounded-lg border border-zinc-200 dark:border-zinc-800 space-y-1">
                    <div className="flex justify-between">
                      <span className="font-semibold text-zinc-500">{t("lbl_box_name", { defaultValue: "Box Name:" })}</span>
                      <span className="font-bold text-zinc-900 dark:text-white">{selectedBox.profile_name}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-semibold text-zinc-500">{t("lbl_current_expiration", { defaultValue: "Current Expiration:" })}</span>
                      <span className="font-bold text-zinc-900 dark:text-white">{formatDate(selectedBox.expiration_date) || "N/A"}</span>
                    </div>
                  </div>
